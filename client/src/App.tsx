  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-7xl px-4 py-5 space-y-5 sm:px-6 sm:py-6 sm:space-y-6">
        {/* Formal Executive & Legal Print Header */}
        <div className="hidden print:block mb-6 p-6 rounded-xl border-2 border-slate-800 bg-white text-slate-900 shadow-sm">
          <div className="flex justify-between items-start border-b-2 border-slate-800 pb-4 mb-4">
            <div>
              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 text-[10px] font-mono font-bold uppercase tracking-widest bg-slate-900 text-white rounded">
                  OFFICIAL AUDIT REPORT
                </span>
                <span className="text-xs font-mono font-semibold text-slate-600">
                  REF ID: REP-{data?.change.period || "2025-11"}-{company}-89F2
                </span>
              </div>
              <h1 className="text-2xl font-bold font-display text-slate-900 mt-2">
                EXECUTIVE KPI STORYTELLING & AUDIT DECK
              </h1>
              <p className="text-xs font-medium text-slate-700 mt-1">
                Subject Entity: <strong className="text-slate-900">{companyName} ({company})</strong> · Target Metric: <strong className="text-slate-900">{data?.meta.name}</strong> · Region: <strong className="text-slate-900">{regionName} ({region})</strong>
              </p>
            </div>
            <div className="text-right font-mono text-xs text-slate-700 space-y-1">
              <div><span className="text-slate-500">SECURITY:</span> <strong>CONFIDENTIAL // FOR INTERNAL USE ONLY</strong></div>
              <div><span className="text-slate-500">GENERATED:</span> {new Date().toISOString().slice(0, 10)} {new Date().toLocaleTimeString()}</div>
              <div><span className="text-slate-500">ANOMALY PERIOD:</span> <strong>{data?.change.period}</strong></div>
              <div><span className="text-slate-500">STATUS:</span> <strong className="text-emerald-700">DETERMINISTIC VERIFIED ✅</strong></div>
            </div>
          </div>

          <div className="text-[11px] leading-relaxed text-slate-700 bg-slate-100 p-3 rounded-lg border border-slate-300">
            <strong>LEGAL & COMPLIANCE PROVENANCE STATEMENT:</strong> This document contains an official, non-hallucinating statistical audit breakdown generated by the <em>KPI Storytelling Engine</em>. All anomaly z-scores, driver margin bridges, vector document embeddings, and action plans are deterministically calculated from database records. Artificial Intelligence output is strictly restricted to plain-language formatting of mathematical facts.
          </div>
        </div>
        ...
        {/* Formal Legal Print Footer */}
        <div className="hidden print:flex mt-8 pt-4 border-t-2 border-slate-800 text-[10px] font-mono text-slate-700 justify-between items-center">
          <div>Signal Room KPI Storytelling System · Audit Verification Hash: <span className="text-slate-900 font-semibold">e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855</span></div>
          <div>Page 1 of 1 · Executive Eyes Only</div>
        </div>
      </main>
    </div>
  );
